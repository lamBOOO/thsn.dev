...
export default function Badge({ icon, left, right, lc, rc, textcolor, textcolor2, link }: any) {
  const badge =
    <span className={`border-gray-500 hover:border-black ${textcolor}`}>
      ...
      <span className={`border-inherit border-t border-r border-b text-xs inline-flex items-center rounded-tr rounded-br px-2 py-0.5 ${rc} ${textcolor2 != undefined ? textcolor2 : ""} font-mono`}>
        {right}
      </span>
    </span>
  if (link != undefined) {
